...
#awk 'NF && !/\[|\]/ && !/#/ {print "nftset=/" $1 "/4#inet#fw4#vpn_domains"}' $SP/mydomains.txt $SP/geoblock.txt $SP/meta.txt $SP/tiktok.txt $SP/telegram.txt $SP/hosts-google.txt $SP/youtube.txt $sp/WhatsApp.txt $sp/discord.txt >$DP/domains.lst
awk 'NF && !/\[|\]/ && !/#/ {print "nftset=/" $1 "/4#inet#fw4#vpn_domains"}' $SP/mydomains.txt $SP/geoblock.txt $SP/meta.txt $SP/tiktok.txt $SP/telegram.txt $SP/youtube.txt $sp/WhatsApp.txt $sp/discord.txt >$DP/domains.lst
echo "\n OK \n"